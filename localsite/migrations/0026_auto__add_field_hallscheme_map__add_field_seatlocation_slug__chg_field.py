...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding field 'HallScheme.map'
        db.add_column('localsite_hallscheme', 'map', self.gf('django.db.models.fields.TextField')(default='', blank=True), keep_default=False)

        # Adding field 'SeatLocation.slug'
        db.add_column('localsite_seatlocation', 'slug', self.gf('django.db.models.fields.SlugField')(db_index=True, default='', max_length=50, blank=True), keep_default=False)

        # Changing field 'SeatLocation.x_position'
        db.alter_column('localsite_seatlocation', 'x_position', self.gf('django.db.models.fields.IntegerField')(null=True))

        # Changing field 'SeatLocation.y_position'
        db.alter_column('localsite_seatlocation', 'y_position', self.gf('django.db.models.fields.IntegerField')(null=True))

        # Adding unique constraint on 'SeatLocation', fields ['section', 'slug']
        db.create_unique('localsite_seatlocation', ['section_id', 'slug'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'SeatLocation', fields ['section', 'slug']
        db.delete_unique('localsite_seatlocation', ['section_id', 'slug'])

        # Deleting field 'HallScheme.map'
        db.delete_column('localsite_hallscheme', 'map')

        # Deleting field 'SeatLocation.slug'
        db.delete_column('localsite_seatlocation', 'slug')

        # User chose to not deal with backwards NULL issues for 'SeatLocation.x_position'
        raise RuntimeError("Cannot reverse this migration. 'SeatLocation.x_position' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'SeatLocation.y_position'
        raise RuntimeError("Cannot reverse this migration. 'SeatLocation.y_position' and its values cannot be restored.")
    ...
    models = {
        ...
        'localsite.seatgroup': {
            'Meta': {'ordering': "['hallscheme', 'name']", 'unique_together': "(('hallscheme', 'name'),)", 'object_name': 'SeatGroup'},
        ...
        'localsite.seatlocation': {
            'Meta': {'ordering': "['section', 'row', 'col']", 'unique_together': "(('section', 'row', 'col'), ('section', 'slug'))", 'object_name': 'SeatLocation'},
