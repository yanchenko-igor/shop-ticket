...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding unique constraint on 'HallScheme', fields ['name']
        db.create_unique('localsite_hallscheme', ['name'])

        # Adding unique constraint on 'Hall', fields ['city', 'name']
        db.create_unique('localsite_hall', ['city_id', 'name'])

        # Adding unique constraint on 'SeatLocation', fields ['hallscheme', 'group', 'col', 'row']
        db.create_unique('localsite_seatlocation', ['hallscheme_id', 'group_id', 'col', 'row'])

        # Adding unique constraint on 'City', fields ['name']
        db.create_unique('localsite_city', ['name'])

        # Adding unique constraint on 'EventDate', fields ['event', 'datetime']
        db.create_unique('localsite_eventdate', ['event_id', 'datetime'])

        # Adding unique constraint on 'Ticket', fields ['product', 'seat', 'datetime']
        db.create_unique('localsite_ticket', ['product_id', 'seat_id', 'datetime_id'])

        # Adding unique constraint on 'Ticket', fields ['seat', 'parent', 'datetime']
        db.create_unique('localsite_ticket', ['seat_id', 'parent_id', 'datetime_id'])

        # Adding unique constraint on 'SeatGroupPrice', fields ['group', 'event']
        db.create_unique('localsite_seatgroupprice', ['group_id', 'event_id'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'SeatGroupPrice', fields ['group', 'event']
        db.delete_unique('localsite_seatgroupprice', ['group_id', 'event_id'])

        # Removing unique constraint on 'Ticket', fields ['seat', 'parent', 'datetime']
        db.delete_unique('localsite_ticket', ['seat_id', 'parent_id', 'datetime_id'])

        # Removing unique constraint on 'Ticket', fields ['product', 'seat', 'datetime']
        db.delete_unique('localsite_ticket', ['product_id', 'seat_id', 'datetime_id'])

        # Removing unique constraint on 'EventDate', fields ['event', 'datetime']
        db.delete_unique('localsite_eventdate', ['event_id', 'datetime'])

        # Removing unique constraint on 'City', fields ['name']
        db.delete_unique('localsite_city', ['name'])

        # Removing unique constraint on 'SeatLocation', fields ['hallscheme', 'group', 'col', 'row']
        db.delete_unique('localsite_seatlocation', ['hallscheme_id', 'group_id', 'col', 'row'])

        # Removing unique constraint on 'Hall', fields ['city', 'name']
        db.delete_unique('localsite_hall', ['city_id', 'name'])

        # Removing unique constraint on 'HallScheme', fields ['name']
        db.delete_unique('localsite_hallscheme', ['name'])
    ...
    models = {
        ...
        'localsite.eventdate': {
            'Meta': {'unique_together': "(('event', 'datetime'),)", 'object_name': 'EventDate'},
            'datetime': ('django.db.models.fields.DateTimeField', [], {}),
            'event': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'dates'", 'to': "orm['localsite.Event']"}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'})
        },
        'localsite.hall': {
            'Meta': {'unique_together': "(('name', 'city'),)", 'object_name': 'Hall'},
        ...
        'localsite.seatgroup': {
            'Meta': {'unique_together': "(('hallscheme', 'name'),)", 'object_name': 'SeatGroup'},
        ...
        'localsite.seatgroupprice': {
            'Meta': {'unique_together': "(('group', 'event'),)", 'object_name': 'SeatGroupPrice'},
        ...
        'localsite.seatlocation': {
            'Meta': {'unique_together': "(('hallscheme', 'group', 'row', 'col'),)", 'object_name': 'SeatLocation'},
        ...
        'localsite.ticket': {
            'Meta': {'unique_together': "(('product', 'datetime', 'seat'), ('parent', 'datetime', 'seat'))", 'object_name': 'Ticket'},
