...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Deleting field 'SeatGroup.section'
        db.delete_column('localsite_seatgroup', 'section_id')

        # Adding field 'SeatGroup.hallscheme'
        db.add_column('localsite_seatgroup', 'hallscheme', self.gf('django.db.models.fields.related.ForeignKey')(default=1, related_name='groups', to=orm['localsite.HallScheme']), keep_default=False)

        # Adding unique constraint on 'SeatGroup', fields ['hallscheme', 'name']
        db.create_unique('localsite_seatgroup', ['hallscheme_id', 'name'])

        # Adding field 'SeatLocation.section'
        db.add_column('localsite_seatlocation', 'section', self.gf('django.db.models.fields.related.ForeignKey')(default=1, related_name='seats', to=orm['localsite.SeatSection']), keep_default=False)

        # Adding unique constraint on 'SeatLocation', fields ['section', 'col', 'row']
        db.create_unique('localsite_seatlocation', ['section_id', 'col', 'row'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'SeatLocation', fields ['section', 'col', 'row']
        db.delete_unique('localsite_seatlocation', ['section_id', 'col', 'row'])

        # Removing unique constraint on 'SeatGroup', fields ['hallscheme', 'name']
        db.delete_unique('localsite_seatgroup', ['hallscheme_id', 'name'])

        # User chose to not deal with backwards NULL issues for 'SeatGroup.section'
        raise RuntimeError("Cannot reverse this migration. 'SeatGroup.section' and its values cannot be restored.")
        ...
        # Deleting field 'SeatLocation.section'
        db.delete_column('localsite_seatlocation', 'section_id')
    ...
    models = {
        ...
        'localsite.seatlocation': {
            'Meta': {'ordering': "['group', 'row', 'col']", 'unique_together': "(('group', 'row', 'col'), ('section', 'row', 'col'))", 'object_name': 'SeatLocation'},
