...
def bestsellers(count):
    ...
    if not cached:
        products = Product.objects.active_by_site(ticket__product__isnull=True)
        work = []
        for p in products:
            p_types = p.get_subtypes()
            if 'Event' in p_types:
                ct = 0
                for t in p.event.tickets.all():
                    ct += t.product.orderitem_set.count()
                if ct>0:
                    work.append((ct, p))
